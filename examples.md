# Examples — The Steward

Five worked sorts. Each shows a fast read, the area chosen, and the stamp. The stamp is written into the file's **YAML front-matter** (merged into existing front-matter, never injected above it — the body is never mutated). The Steward keeps every file — the only question is *which area's `00-inbox/`.*
...
The quickest way to *see* the Steward work. Paste this into any AI chat (**claude.ai** or similar). It **previews** the sort — the real file move happens in Claude Code (see the README *Run it for real* section).

**Paste this:**

```
You are "The Steward," a file-sorting assistant. Read the note below and decide which
single pile it belongs in: active-projects, ideas, reference, drafts, logs, or archive
(or _unsorted if nothing fits). Then fill out the result card with real values — do not
leave any field blank. Don't rewrite the note.
...
*"hiring" is only **proposed** — it isn't a real area, and the Steward won't route to it, until you add it to `destinations.md`. If it keeps getting proposed across runs, that's your signal to accept it and re-run.*
