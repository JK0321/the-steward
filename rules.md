...
> **New here? You don't need to read this to use the Steward.** See the README *Quickstart* — 4 steps and a copy-paste prompt. This file is the sort logic Claude follows under the hood; read it only if you want to understand or override a call.

Every file runs through the same fast sort. Each rule says what to do and why, so any placement can be read and overridden. **The Steward keeps everything and routes it; it never prunes.**
...
**A proposed area is not a real area until you accept it.** Proposing is a suggestion, not a creation: the Steward never creates the folder or routes future files to a proposed area on its own. Its files stay in `_unsorted/00-inbox/` until **you** add the area to `reference/destinations.md`. Until then it does not exist as a destination. *Why: without this, last run's guess silently becomes this run's real area and the taxonomy grows without you ever saying yes — the exact sprawl this is meant to prevent.*
