...
Every option below runs **the Steward's real rules** (`rules.md` + `reference/destinations.md`) — nothing is faked or typed in by hand. The point isn't "will an AI follow instructions" (it always will); it's *does the folder do the work?* So each rung loads the actual files.
...
real projects under daily noise. The Steward refuses the obvious read and routes it to `logs`.
...
into the nearest bucket to avoid looking unsure. The Steward parks it in `_unsorted/` *and
...
back with "where should this go?" The Steward routes it to the area it's *most* about, notes the
other, and moves on. It decides.

That's why it's a steward and not a search box: it makes the fast, repetitive judgment call for
