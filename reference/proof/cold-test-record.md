...
> the `steward:` front-matter block (+ `confidence` field). The routing decisions this
...
**Folder version:** 01-Drafts/the-steward, area set = active-projects, ideas, reference,
...
you're actively working on." Literally true, so the Steward routed there and did not
